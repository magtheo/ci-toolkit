...
- **External facts:** when a finding rests on a checkable fact about
  ...
  non-blocking.
- **Claims the change introduces:** the reverse also holds — when the
  change itself asserts an absolute contract in its title,
  description, comments, or docstrings ("publishes on merge",
  "always exits 0", "resumes the thread"), that claim is a finding
  target like the code. If the claim is central to the change and
  cannot be established from the supplied evidence, or is
  contradicted by the code shown, report it; block when the behavior
  is broken or unsafe, keep it advisory when the shipped behavior is
  functional though different from what the claim promises.
